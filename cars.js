...
{
    const saleProfit = []
    carData.forEach((sale) => {
        saleProfit.push(sale.gross_profit) 
    })
    const totalProfit = (a, b) => {
        return a + b
    }
    console.log(`${saleProfit.reduce(totalProfit)}`)
    }
{
    carData.forEach((date => {
        carData.sale.purchase_date

    }))
    const highestMonthSales = () => {
